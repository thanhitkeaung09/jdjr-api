...
test('If there are no app keys, it is not possible to get all levels', function (): void {
    withHeaders([
        'app-id' => null,
        'app-secrete' => null,
    ]);
    ...
    getJson(
        uri: action(AllLevelsController::class),
    )
...
test('If the app keys are outdated, it is not possible to get all levels', function (): void {
...
test('Unauthenicated user cannot get all levels', function (): void {
    getJson(
        uri: action(AllLevelsController::class),
    )
...
it('returns the correct payload when levels are empty in db', function (): void {
    Sanctum::actingAs(User::factory()->create());
